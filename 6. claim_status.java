
import java.util.*;


/**
 * Class claim
 */
public class claim {

  //
  // Fields
  //

  private String claimnumber;
  private date datefiled;
  private money premiumamount;
  
  //
  // Constructors
  //
  public claim () { };
  
  //
  // Methods
  //


  //
  // Accessor methods
  //

  /**
   * Set the value of claimnumber
   * @param newVar the new value of claimnumber
   */
  private void setClaimnumber (String newVar) {
    claimnumber = newVar;
  }

  /**
   * Get the value of claimnumber
   * @return the value of claimnumber
   */
  private String getClaimnumber () {
    return claimnumber;
  }

  /**
   * Set the value of datefiled
   * @param newVar the new value of datefiled
   */
  private void setDatefiled (date newVar) {
    datefiled = newVar;
  }

  /**
   * Get the value of datefiled
   * @return the value of datefiled
   */
  private date getDatefiled () {
    return datefiled;
  }

  /**
   * Set the value of premiumamount
   * @param newVar the new value of premiumamount
   */
  private void setPremiumamount (money newVar) {
    premiumamount = newVar;
  }

  /**
   * Get the value of premiumamount
   * @return the value of premiumamount
   */
  private money getPremiumamount () {
    return premiumamount;
  }

  //
  // Other methods
  //

  /**
   */
  public void getstatus()
  {
  }


  /**
   */
  public void processclaim()
  {
  }


}



/**
 * Class claim_status
 */
public class claim_status {

  //
  // Fields
  //

  private void pending;
  private void approved;
  private void rejected;
  
  //
  // Constructors
  //
  public claim_status () { };
  
  //
  // Methods
  //


  //
  // Accessor methods
  //

  /**
   * Set the value of pending
   * @param newVar the new value of pending
   */
  private void setPending (void newVar) {
    pending = newVar;
  }

  /**
   * Get the value of pending
   * @return the value of pending
   */
  private void getPending () {
    return pending;
  }

  /**
   * Set the value of approved
   * @param newVar the new value of approved
   */
  private void setApproved (void newVar) {
    approved = newVar;
  }

  /**
   * Get the value of approved
   * @return the value of approved
   */
  private void getApproved () {
    return approved;
  }

  /**
   * Set the value of rejected
   * @param newVar the new value of rejected
   */
  private void setRejected (void newVar) {
    rejected = newVar;
  }

  /**
   * Get the value of rejected
   * @return the value of rejected
   */
  private void getRejected () {
    return rejected;
  }

  //
  // Other methods
  //

}
